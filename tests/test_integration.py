...
from photobatch.Processing.data_processor import FileJob, _process_single_file, process_files
...
def test_process_single_file_skips_when_synchronization_fails(monkeypatch, caplog, tmp_path):
    ...
    args = FileJob(
        ...
        event_sheet_path=str(event_sheet_path),
        output_options=[],
        event_window_prior=0.5,
        event_window_follow=0.5,
        trial_start_stage=["Display Image"],
        trial_end_stage=["Reward Collected Start ITI"],
        iti_prior_trial=0.5,
        center_z="whole",
        center_method="mean",
        scale_median=False,
